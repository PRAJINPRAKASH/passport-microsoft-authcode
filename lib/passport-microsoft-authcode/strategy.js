...
MicrosoftAuthCodeStrategy.prototype.authenticate = function(req, options) {
  ...
  self._exchangeAuthCode(authCode, function(err, accessToken, refreshToken, resultsJson) {
    ...
    if (err) {console.log('err>>',err); return self.fail(err); };

    self._loadUserProfile(accessToken, function(err, profile) {
      
      if (err) { console.log('err2>>',err);return self.fail(err); };
      
      function verified(err, user, info) {
        
        if (err) { console.log('err3>>',err);return self.error(err); }
        if (!user) { console.log('err4>>',user);return self.fail(info); }
 ...
 * @api private
 */
MicrosoftAuthCodeStrategy.prototype._exchangeAuthCode = function(authCode, done) {
  var params = {
...
MicrosoftAuthCodeStrategy.prototype.userProfile = function(accessToken, done) {
  this._oauth2.get('https://graph.microsoft.com/v1.0/me', accessToken, function (err, body, res) {
    ...
    try {
      ...
      profile.emails = [{ value: json.mail }];
 ...
 * @api private
 */
MicrosoftAuthCodeStrategy.prototype._loadUserProfile = function(accessToken, done) {
  var self = this;
